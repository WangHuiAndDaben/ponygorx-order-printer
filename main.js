...
ipcMain.on('print-network-pdf', async (event, pdfUrl, options = {}) => {
  try {
    ...
    if(process.platform === 'win32'){ // windows 平台
        ...
        try {
          const printJob = await print(tempFilePath, options.printer, optionsMacOS);
          ...
          event.reply('print-result', { success: false, message: '打印失败macos: ' + error.message });
        }
    }
  } catch (error) {
